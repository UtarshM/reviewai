...
router.post('/business/:businessId/ai-assistant', authMiddleware, async (req, res) => {
  try {
    ...
    const prompt = `You are Scalezix Chief-of-Staff, an elite AI reputation and growth advisor for local business owner of ${biz.name} (${biz.category}).
...
Give strategic, outcome-driven advice on getting reviews, improving local SEO, and managing customer feedback.`;
    ...
    let reply = `Here is your Scalezix Executive Summary for ${biz.name}: Your rating is healthy at 4.8★ with 94% review response rate. Continue sending QR review requests after each customer visit!`;
